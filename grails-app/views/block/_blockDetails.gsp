...
<div class="blockRow">
	<div onclick="showHideBlockInfo(this);" class="blockHeader open"><h3 title="Block created on <g:formatDate date='${block.dateCreated}' format='MM/dd/yyyy h:mm a'/> by ${block.userCreated}" >${block}</h3></div>
	<div class="blockInfo">
		...
		<ul class="errors" role="alert">
			<g:eachError bean="${block}" var="error">
		...
		<span class="editBlock">
			<g:form name="${'editBlockForm_' + block.id}">
				<g:remoteLink controller="block" action="edit" id="${block.id}" update="${'block' + block.id}" onSuccess="showBlockEditForm()" >Edit Block</g:remoteLink>
			</g:form>
		</span>
		<div class="stainInfo">			
			<h3>Stains</h3>
